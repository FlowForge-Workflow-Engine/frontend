...
export default function UsersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.get(`/api/v1/users?page=${page}&limit=20`),
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["users"] }); setAddOpen(false); reset(); toast.success("User created"); },
  ...
  const assignRoleMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["users"] }); setRoleDialogUser(null); toast.success("Role assigned"); },
    onError: (e) => toast.error(getErrorMessage(e)),
  });
  ...
  const columns: ColumnDef<User>[] = [
      ...
      cell: ({ row }) => (
        <div className="flex gap-1 flex-wrap">
          {row.original.roles?.map((r) => (
            <span key={r.id} className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary">{r.name}</span>
          ))}
        </div>
      ),
    },
    {
      header: "Status",
      cell: ({ row }) => (
        <span className={`inline-flex items-center gap-1.5 text-xs ${row.original.isActive ? "text-status-active" : "text-muted-foreground"}`}>
          <span className={`w-2 h-2 rounded-full ${row.original.isActive ? "bg-status-active" : "bg-muted-foreground"}`} />
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-1">
          <Button size="sm" variant="ghost" onClick={() => { setRoleDialogUser(row.original); setSelectedRoleId(""); }}>
  ...
  return (
    <div>
      ...
      <DataTable columns={columns} data={data?.items ?? []} totalCount={data?.count} page={page} pageSize={20} onPageChange={setPage} />

      {/* Assign Role Dialog */}
      <Dialog open={!!roleDialogUser} onOpenChange={(open) => !open && setRoleDialogUser(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Assign Role to {roleDialogUser?.firstName}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <Button className="w-full" disabled={!selectedRoleId || assignRoleMut.isPending} onClick={() => assignRoleMut.mutate({ userId: roleDialogUser!.id, roleId: selectedRoleId })}>
